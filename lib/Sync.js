...
Function.prototype.run = function(){
    ...
    function Factory(){
        ...
        var exec = function(callback, context, _continue){
           
            var hasCallback = false
            var index
            for(var i = 0; i < this.args.length; i++){
                if(this.args[i] === callback) {
                    hasCallback = true
                    index = i
                    callback = this.args[i]
                }
            }
            
            var Factory = function(){ return callback.bind(this) }
            Factory.prototype._continue = _continue

            callback = new Factory()

            if(!hasCallback) this.args.push(callback)
            else this.args[index] = callback
